...
exports.WalletClient = exports.MAX_U64_BIG_INT = void 0;
...
class WalletClient {
    constructor(node_url, faucet_url) {
        this.faucetClient = new aptos_1.FaucetClient(node_url, faucet_url);
        this.aptosClient = new aptos_1.AptosClient(node_url);
        this.tokenClient = new aptos_1.TokenClient(this.aptosClient);
    }
    ...
    async importWallet(code) {
        ...
        for (let i = 0; i < MAX_ACCOUNTS; i += 1) {
            ...
            for (let j = 0; j < ADDRESS_GAP; j += 1) {
                ...
                const account = aptos_1.AptosAccount.fromDerivePath(derivationPath, code);
                if (j === 0) {
                    address = aptos_1.HexString.ensure(account.address()).toShortString();
    ...
    createNewAccount(code, index = 0) {
        ...
        const account = aptos_1.AptosAccount.fromDerivePath(derivationPath, code);
        const address = aptos_1.HexString.ensure(account.address()).toShortString();
        return {
            derivationPath,
            address,
            publicKey: account.pubKey().toString(),
    ...
    static getTransactionDeserialized(bytes) {
        const deserializer = new aptos_1.BCS.Deserializer(bytes);
        return aptos_1.TxnBuilderTypes.RawTransaction.deserialize(deserializer);
    }
    /**
    ...
    static getAccountFromMnemonic(code) {
        return aptos_1.AptosAccount.fromDerivePath(`m/44'/${COIN_TYPE}'/0'/0'/0'`, code);
    }
    /**
    ...
    static getAccountFromMetaData(code, metaData) {
        return aptos_1.AptosAccount.fromDerivePath(metaData.derivationPath, code);
    }
    /**
     ...
     * @returns transaction hash
     */
    async createCollection(account, name, description, uri, maxAmount = exports.MAX_U64_BIG_INT, extraArgs) {
     ...
     * @returns transaction hash
     */
    async createToken(account, collection_name, name, description, supply, uri, max = exports.MAX_U64_BIG_INT, royalty_payee_address = account.address(), royalty_points_denominator = 0, royalty_points_numerator = 0, property_keys = [], property_values = [], property_types = [], extraArgs) {
    ...
    static generateBCSTransaction(account, rawTxn) {
        return Promise.resolve(aptos_1.AptosClient.generateBCSTransaction(account, rawTxn));
    }
    static generateBCSSimulation(account, rawTxn) {
        return Promise.resolve(aptos_1.AptosClient.generateBCSSimulation(account, rawTxn));
    }
    ...
    async getTokens(address, limit, depositStart, withdrawStart) {
        ...
        return await this.getTokensFromTokenIds(tokenIds);
    }
    async getTokensFromTokenIds(tokenIds) {
        const tokens = [];
    ...
    async getTokensFromTokenIdsWithCount(tokenIdObjects) {
        const tokens = [];
        await Promise.all(tokenIdObjects.map(async (tokenIdObject) => {
            let tokenId = tokenIdObject.data;
            ...
            try {
                ...
                token.collection = tokenId.data.token_data_id.collection;
                tokens.push({ token, tokenId, count, sequence_number: tokenId.sequence_number });
            }
            catch (e) {
                // Errors happening because of token handle not found will lead here
    ...
    async getCollection(address, collectionName) {
        ...
        const collection = await this.aptosClient.getTableItem(accountResource.data.collection_data.handle, tableItemRequest);
        return collection;
    }
    ...
    async registerCoin(account, coin_type_path) {
        const token = new aptos_1.TxnBuilderTypes.TypeTagStruct(aptos_1.TxnBuilderTypes.StructTag.fromString(coin_type_path));
        const entryFunctionPayload = new aptos_1.TxnBuilderTypes.TransactionPayloadEntryFunction(aptos_1.TxnBuilderTypes.EntryFunction.natural("0x1::managed_coin", "register", [token], []));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_1.AptosClient.generateBCSTransaction(account, rawTxn);
    ...
    dst_address, amount) {
        const token = new aptos_1.TxnBuilderTypes.TypeTagStruct(aptos_1.TxnBuilderTypes.StructTag.fromString(coin_type_path));
        const entryFunctionPayload = new aptos_1.TxnBuilderTypes.TransactionPayloadEntryFunction(aptos_1.TxnBuilderTypes.EntryFunction.natural("0x1::managed_coin", "mint", [token], [
            aptos_1.BCS.bcsToBytes(aptos_1.TxnBuilderTypes.AccountAddress.fromHex(aptos_1.HexString.ensure(dst_address).toString())),
    ...
    to_address, amount) {
        const token = new aptos_1.TxnBuilderTypes.TypeTagStruct(aptos_1.TxnBuilderTypes.StructTag.fromString(coin_type_path));
        const entryFunctionPayload = new aptos_1.TxnBuilderTypes.TransactionPayloadEntryFunction(aptos_1.TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [
            aptos_1.BCS.bcsToBytes(aptos_1.TxnBuilderTypes.AccountAddress.fromHex(aptos_1.HexString.ensure(to_address).toString())),
            aptos_1.BCS.bcsSerializeUint64(amount),
        ]));
        const rawTxn = await this.aptosClient.generateRawTransaction(account.address(), entryFunctionPayload);
        const bcsTxn = aptos_1.AptosClient.generateBCSTransaction(account, rawTxn);
    ...
    async publishModule(sender, packageMetadataHex, moduleHex, extraArgs) {
        const txnHash = await this.aptosClient.publishPackage(sender, new aptos_1.HexString(packageMetadataHex).toUint8Array(), [new aptos_1.TxnBuilderTypes.Module(new aptos_1.HexString(moduleHex).toUint8Array())], extraArgs);
        return txnHash;
    }
}
