...
import {MaybeHexString, 
      ...
      AptosAccount,
      HexString,
      BCS, TxnBuilderTypes, OptionalTransactionArgs, Types } from "aptos";
...
export class WalletClient {
   ...
   * @returns list of events
   */
  async getReceivedEvents(address: string, limit?: number, start?: BCS.AnyNumber) {
  ...
  async estimateCost(
    ...
    const change = txnData.changes.filter((ch) => {
      ...
      console.log('ch', ch)
      const write = ch as Types.WriteResource;
      if (
        write.data.type ===
          "0x1::coin::CoinStore<0x1::aptos_coin::AptosCoin>" &&
  ...
  static generateBCSSimulation(
    ...
    return Promise.resolve(
      AptosClient.generateBCSSimulation(account, rawTxn)
    );
  }
  ...
  async getTokensFromTokenIdsWithCount(
    tokenIdObjects: any[]
  ) {
    const tokens = [];
    await Promise.all(
      tokenIdObjects.map(async (tokenIdObject) => {
  ...
  async getCollection(address: string, collectionName: string) {
    ...
    const collection = await this.aptosClient.getTableItem(
      accountResource.data.collection_data.handle,
      tableItemRequest
    );
    return collection;
  }
